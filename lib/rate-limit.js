...
const PRESETS = {
  ...
  // notify: 5/min — "email me when it launches" signups. A human signs up
  // once; 5/min per IP allows typo-retries while blocking list-stuffing bots.
  notify: { windowMs: 60_000, maxRequests: 5 },
  // webScan: 3/min — /api/web/scan and /api/wp/scan are free, unauthenticated,
  // no-signup URL scanners that each spin up a full CLI-engine workspace and
  // dispatch a headless-browser job — at least as expensive as scanRun, but
  // previously had no rate limit at all. Same shape as scanRun.
  webScan: { windowMs: 60_000, maxRequests: 3 },
  // feedback: 10/min — the one-question prompt under a scan result and the
  // per-finding "Wrong?" link. A reviewer flagging findings one by one on a
  // noisy scan stays under it; a bot cannot flood feedback_events or the
  // GitHub issues the escalation opens. The route layers the Postgres count
  // of this IP's rows on top via dbBackedFn (feedback-store.countRecentByIp),
  // so the limit holds across instances, not only inside one warm process.
  feedback: { windowMs: 60_000, maxRequests: 10 },
  // billingPortal: 3/min — each request can trigger an outbound email and a
  // Stripe API call. A real customer needs exactly one; 3 allows typo retries
  // while blocking email-bombing someone else's inbox via our sender.
  billingPortal: { windowMs: 60_000, maxRequests: 3 },
};
