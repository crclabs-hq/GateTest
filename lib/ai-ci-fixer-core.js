...
const fixerClient = require('./ai-ci-fixer-claude');
    ...
    const req = transport.request(reqOpts, (res) => {
      ...
      res.on('end', () => {
        ...
        if (raw && (res.headers['content-type'] || '').includes('json')) {
          try { parsed = JSON.parse(raw); } catch { /* error-ok — a body that claims JSON but is not stays raw; the caller reads it as text */ }
        }
...
function buildPrBody({ runUrl, logExcerpt, attempt, model, diagnosis, patches }) {
  ...
  if (Array.isArray(patches) && patches.length > 0) {
    ...
    filesBlock += '\nThe **AST** and **Rule** tags are deterministic flywheel fixes — no AI tokens spent. ' +
                  '**AI** tags are bespoke patches written by the AI fix engine.\n\n';
  }
...
function readEnv(env = process.env) {
  ...
  return {
    ...
    model:       env.CLAUDE_MODEL || fixerClient.DEFAULT_MODEL,
  };
}
...
module.exports = {
  ...
  ANTHROPIC_API_HOST:    fixerClient.ANTHROPIC_API_HOST,
  ANTHROPIC_API_PATH:    fixerClient.ANTHROPIC_API_PATH,
  ANTHROPIC_VERSION:     fixerClient.ANTHROPIC_VERSION,
  DEFAULT_MODEL:         fixerClient.DEFAULT_MODEL,
  CLAUDE_TIMEOUT_MS:     fixerClient.CLAUDE_TIMEOUT_MS,
  MAX_INPUT_CHARS:       fixerClient.MAX_INPUT_CHARS,
  CLAUDE_SYSTEM_PROMPT:  fixerClient.CLAUDE_SYSTEM_PROMPT,
  CLAUDE_RETRY_ATTEMPTS: fixerClient.CLAUDE_RETRY_ATTEMPTS,
  CLAUDE_RETRY_DELAYS_MS:fixerClient.CLAUDE_RETRY_DELAYS_MS,
  CLAUDE_RETRY_BUDGET_MS:fixerClient.CLAUDE_RETRY_BUDGET_MS,
  // Logging
  log, logErr,
  // GitHub
  ...
  buildClaudePrompt:  fixerClient.buildClaudePrompt,
  parseClaudeResponse:fixerClient.parseClaudeResponse,
  callClaude:         fixerClient.callClaude,
  normaliseCiLog:     fixerClient.normaliseCiLog,
  // I/O
  ...
  readEnv,
  // Test helpers
  _callClaudeOnce:        fixerClient._callClaudeOnce,
  _isRetryableClaudeError:fixerClient._isRetryableClaudeError,
  _parseRetryAfter:       fixerClient._parseRetryAfter,
};
