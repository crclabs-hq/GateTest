...
/**
 * Run an `npx <args...>` command cross-platform.
 *
 * Windows can't spawnSync a bare "npx" (it's a .cmd shim) without a shell —
 * and as of Node's CVE-2024-27980 fix, spawning a .cmd/.bat file directly
 * (even by full name) throws EINVAL unless shell:true is set. So shell:true
 * is required on Windows. To avoid Node's DEP0190 warning (shell:true +
 * an args array skips shell-escaping), the full command line is built as
 * one string and passed with an empty args array instead — safe here since
 * every caller passes static, space-free tokens (package/binary names,
 * fixed flags), never untrusted input.
 */
function spawnNpx(exec, args, opts) {
  if (process.platform === 'win32') {
    const commandLine = ['npx', ...args].join(' ');
    return exec(commandLine, [], { ...opts, shell: true });
  }
  return exec('npx', args, opts);
}
...
function runWebsiteBuildStep({ cwd, runner } = {}) {
  ...
  const exec = runner || runProcess;
  const r = spawnNpx(exec, ['next', 'build'], { cwd: websiteDir });
...
/**
 * STEP 8 — Lint. `npm run lint` (eslint over src/, bin/, lib/, integrations/).
 * Pass = exit 0.
 */
function runLintStep({ cwd, runner } = {}) {
  const root = cwd || process.cwd();
  const exec = runner || runProcess;
  const r = spawnNpx(exec, ['eslint', 'src', 'bin', 'lib', 'integrations'], { cwd: root });

  if (r.spawnError) {
    return {
      ok: false,
      summary: `Could not run lint: ${r.spawnError.message || r.spawnError}`,
      durationMs: r.durationMs,
      stdout: r.stdout,
      stderr: r.stderr,
      exitCode: r.exitCode,
  ...
  const out = r.stdout + r.stderr;
  const problemMatch = out.match(/(\d+)\s+problems?\s+\((\d+)\s+errors?,\s+(\d+)\s+warnings?\)/);
  const errorCount = problemMatch ? Number(problemMatch[2]) : (r.exitCode === 0 ? 0 : null);
  const warningCount = problemMatch ? Number(problemMatch[3]) : null;

  let summary;
  if (r.exitCode === 0) {
    summary = warningCount ? `clean (${warningCount} warning(s))` : 'clean';
  } else {
    summary = errorCount != null ? `${errorCount} error(s)` : `exit ${r.exitCode}`;
  }
...
const ALL_STEPS = [
  ...
  { number: 8, key: 'lint', name: 'Lint', run: runLintStep },
];
