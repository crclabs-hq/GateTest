...
function buildDiagnosisPrompt(logExcerpt, files) {
  const cleanLog = fixerClient.normaliseCiLog(logExcerpt || '');
...
async function diagnose({ apiKey, model, logExcerpt, files, transport, callClaude }) {
  const call = callClaude || fixerClient.callClaude;
  ...
  try {
    const text = await call({
      apiKey,
      model: model || fixerClient.DEFAULT_MODEL,
      system: DIAGNOSIS_SYSTEM_PROMPT,
      user,
      timeoutMs: fixerClient.CLAUDE_TIMEOUT_MS,
      transport,
    });
    return parseDiagnosis(text);
  } catch (err) {
    try { process.stderr.write(`[ai-ci-fixer] diagnose failed: ${err && err.message}\n`); } catch { /* error-ok — stderr may be closed; the failure is already returned as ok:false */ }
    return { rootCause: '', plan: '', confidence: 0, confidenceReason: '', ok: false };
  }
}
