...
function _log(msg) {
  try { process.stderr.write(`[ai-ci-fixer] ${msg}\n`); } catch { /* error-ok — stderr may be closed; a log line is never worth failing the operation */ }
}
...
function parseClaudeResponse(text, originalFilesArray = []) {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (allowlistMode && !allowedPaths.has(normalizedTarget)) {
      _log(`Security Reject: the AI provider attempted to write unrequested file: ${normalizedTarget}`);
      continue;
    }
...
function _callClaudeOnce({ apiKey, model, system, user, timeoutMs = CLAUDE_TIMEOUT_MS, transport }) {
  return new Promise((resolve, reject) => {
    ...
    const req = t.request(reqOpts, (res) => {
      ...
      res.on('end', () => {
        ...
        if (res.statusCode !== 200) {
          const err = new Error(`AI provider API ${res.statusCode}: ${raw.slice(0, 200)}`);
        ...
        try {
          ...
          reject(new Error(`Failed to parse AI provider response: ${err.message}`));
        }
      });
    });
    req.on('error', (err) => {
    ...
    const timer = setTimeout(() => {
      const err = new Error(`AI provider call timed out after ${timeoutMs}ms`);
      err._code = 'ETIMEDOUT';
      req.destroy(err);
    }, timeoutMs);
...
async function callClaude(opts) {
  ...
  for (let i = 0; i < CLAUDE_RETRY_ATTEMPTS; i++) {
    try {
      ...
      _log(`AI provider retry ${i + 1}/${CLAUDE_RETRY_ATTEMPTS - 1} after ${delay}ms (status=${err._status || ''} code=${err._code || ''})`);
      await _sleep(delay, sleepFn);
    }
  }
  throw lastErr || new Error('AI provider call failed after retries');
}
