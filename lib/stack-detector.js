...
function readSafely(filePath, fileContents) {
  if (fileContents) {
    ...
    // On Windows path.join uses backslashes; fileContents keys may use forward slashes
    const normalized = filePath.replace(/\\/g, '/');
    if (normalized !== filePath && Object.prototype.hasOwnProperty.call(fileContents, normalized)) {
      return fileContents[normalized];
    }
  }
  try {
...
function detectCi(root, fileContents) {
  ...
  if (readSafely(path.join(root, '.github/workflows/ci.yml'), fileContents)
    ...
    } catch { /* error-ok — unreadable .github directory — detection continues with the other CI markers */ }
  }
