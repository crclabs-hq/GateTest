#!/usr/bin/env node
/**
 * GateTest MCP Server — @gatetest-ai/mcp-server
 *
 ...
 *   claude mcp add gatetest -- npx -y @gatetest-ai/mcp-server
 *
 * Or manually in settings.json / .claude/mcp_servers.json:
 *   {
 *     "mcpServers": {
 *       "gatetest": {
 *         "command": "npx",
 *         "args": ["-y", "@gatetest-ai/mcp-server"]
 *       }
 *     }
 *   }
 *
 ...
 *   compare_repos    — cross-repo lookup (uses memory store)
 */
...
// Core GateTest engine from the gatetest npm package
const { GateTest } = require('gatetest');
const { aiFix } = require('gatetest/src/core/ai-fix-engine.js');
const { MemoryStore } = require('gatetest/src/core/memory.js');

// Bundled utilities (copied from website/app/lib at publish time)
const { composePrBody } = require(resolve(__dirname, '..', 'pr-composer.js'));
const { diagnoseFinding } = require(resolve(__dirname, '..', 'nuclear-diagnoser.js'));
...
const TOOLS = [
  {
    name: 'scan_local',
    description:
      "Scan a local directory with GateTest's 91-module engine. " +
      ...
      'for all 91 modules. Optionally pass a list of specific module names.',
    inputSchema: {
      type: 'object',
      properties: {
        path: {
  ...
  },
  {
    name: 'list_modules',
    description:
      'List all 91 GateTest modules with their names and descriptions. ' +
...
function formatScanResult(result) {
  ...
  lines.push(`## GateTest Scan — ${blocked ? '❌ BLOCKED' : '✅ PASSED'}`);
  lines.push('');
...
async function handleScanLocal(args) {
  ...
  try {
    ...
    if (modules && Array.isArray(modules) && modules.length > 0) {
      ...
      result = await gt.runSuite(suite || 'standard');
    }
    return {
      content: [
        { type: 'text', text: formatScanResult(result) },
        { type: 'text', text: `\n<details>\n<summary>Full JSON result</summary>\n\n\`\`\`json\n${JSON.stringify(result, null, 2)}\n\`\`\`\n</details>` },
      ],
    };
  } catch (err) {
    return { content: [{ type: 'text', text: `Scan failed: ${err?.message ?? String(err)}` }], isError: true };
  }
}
...
async function handleRunModule(args) {
  ...
  try {
    ...
    return { content: [{ type: 'text', text: `Module run failed: ${err?.message ?? String(err)}` }], isError: true };
  }
}
...
async function handleListModules() {
  try {
    ...
    for (const [name, mod] of allModules) {
      lines.push(`**\`${name}\`** — ${mod.description || mod.name || name}`);
    }
    lines.push('', `Total: ${allModules.size} modules loaded`);
    return { content: [{ type: 'text', text: lines.join('\n') }] };
  } catch (err) {
    return { content: [{ type: 'text', text: `Failed to list modules: ${err?.message ?? String(err)}` }], isError: true };
  }
}
  ...
  try {
    ...
    return {
      content: [{
        ...
        text:
          `## GateTest Health\n\n✅ **Operational**\n\n` +
          `- Package: @gatetest-ai/mcp-server v1.0.0\n` +
    ...
    return { content: [{ type: 'text', text: `Health check failed: ${err?.message ?? String(err)}` }], isError: true };
  }
}
...
async function handleFixIssue(args) {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    return {
      content: [{
        type: 'text',
        text: 'fix_issue requires ANTHROPIC_API_KEY. Set it in your environment and retry.',
      }],
      isError: true,
    };
  }
  try {
    ...
    if (!result || result.fixed !== true) {
      return { content: [{ type: 'text', text: `No fix produced. ${result?.description || 'unknown reason'}` }], isError: true };
    }
    return {
      content: [{
        type: 'text',
    ...
    return { content: [{ type: 'text', text: `fix_issue failed: ${err?.message ?? String(err)}` }], isError: true };
  }
}
...
async function handleComposePr(args) {
  ...
  try {
    const body = composePrBody({ fixes, repoUrl: repoUrl || 'unknown', tier: tier || 'full' });
    return { content: [{ type: 'text', text: body }] };
  } catch (err) {
    return { content: [{ type: 'text', text: `compose_pr failed: ${err?.message ?? String(err)}` }], isError: true };
  }
}
...
async function handleExplainFinding(args) {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    return {
      content: [{
        type: 'text',
        text: 'explain_finding requires ANTHROPIC_API_KEY. Set it in your environment and retry.',
      }],
      isError: true,
    };
  }
  try {
    ...
    return { content: [{ type: 'text', text: `explain_finding failed: ${err?.message ?? String(err)}` }], isError: true };
  }
}
...
async function handleAuditLog(args) {
  ...
  try {
    ...
    return { content: [{ type: 'text', text: `audit_log failed: ${err?.message ?? String(err)}` }], isError: true };
  }
}
...
async function handleCompareRepos(args) {
  ...
  try {
      ...
      return {
        content: [{
          ...
          text:
            ...
            `Need at least 3 prior scans in the local memory store. Currently have ${totalScans}. ` +
            `Run more scans to enable pattern matching.`,
        }],
      };
    }
    ...
    if (top.length === 0) lines.push('(no module breakdown stored in scan history)');
    return { content: [{ type: 'text', text: lines.join('\n') }] };
  } catch (err) {
    return { content: [{ type: 'text', text: `compare_repos failed: ${err?.message ?? String(err)}` }], isError: true };
  }
}
