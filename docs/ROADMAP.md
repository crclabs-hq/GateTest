...
| 37 | **Product-direction pivot (Craig, 2026-07-09)** — after feedback that the "22 MCP tools fix Claude's limitations" pitch is weaker/more perishable than the 120-module engine (Playwright + model upgrades erode it over time), priorities re-sequenced: (1) full concentration on the 120-module scan engine, (2) a new architectural question — GateTest's own API key with customer-chosen model (Sonnet 5 / Opus 4.8 / Fable 5, a natural extension of the `modelForTier()` routing already in `engine-models.js`) vs. bring-your-own-key (BYOK), addressing the "burning credits" market concern — needs its own dedicated planning session (Boss Rule #1 + #7: new capability, potential new stored-credential surface), (3) tidy up the 22 MCP tools for Claude/agent users specifically, (4) pen-testing (Coming Soon marking already shipped 2026-07-09, see modules-data.ts `pen-test` category). An MCP-tier pricing-threshold design ("first 2000 signups at $29/mo, then $49/mo" — counter in `mcp-subscription-store.js` keyed off the Stripe webhook activation point, threshold check in `checkout/route.ts`'s `TIERS["mcp"]`) was fully scoped the same day but shelved, not built, given this re-sequencing. | MEDIUM | OPEN — awaiting Craig's go on the model-choice/BYOK dedicated session; MCP pricing design is ready to build if/when MCP tier work resumes. |
---
