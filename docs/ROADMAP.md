...
| 43 | **Self-scan 2026-07-15 found 3 modules (tls-security, cookie-security, cross-file-taint) flagging their own test fixtures as real findings** — 28 of 29 findings printed in one scan run pointed at `tests/*.test.js`, matching literal sample-payload strings the tests write as fixture data (e.g. `tests/tls-security.test.js` containing the string `'process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0"'` as a JS string literal, not live code). **Fixed same day** (commit `4efe693`): added `BaseModule#_isInsideStringLiteral` + regex-literal-aware `_stripJsStrings` (now shared, was duplicated per-module), plus multi-line template-literal tracking in `cross-file-taint.js`'s sink detection. Verified against the exact files/lines originally flagged — now zero findings on all of them, plus new regression tests. **NOT yet fixed**: the same self-referential pattern exists in at least `redos` (22 warnings on `tests/redos.test.js` — its own ReDoS-payload fixtures), `cronExpression` (13 warnings on `tests/cron-expression.test.js` — its own malformed-cron-expression fixtures), and likely `asyncIteration`. Total scan warnings only dropped 537→515 despite the 3-module fix because these other modules carry the same noise. The `_isInsideStringLiteral`/template-tracking pattern built tonight is directly reusable — same shape of fix, different modules. | MEDIUM | OPEN — apply the same fix pattern to redos.js, cron-expression.js, async-iteration.js (and audit remaining regex/text-matching modules for the same class of self-match). |
