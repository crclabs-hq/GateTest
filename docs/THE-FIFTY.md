...
| 08 | Retire any rule above 20% FP that can't be fixed | partial — mechanism landed, awaiting field data | `GET /api/noise` publishes the worst-first per-rule table (`sampleSize`, `silencedRate`, `candidate`, three-state `status`: ok / not-enough-data / ledger-unavailable — never an empty 200 that reads as "no noisy rules"); `src/core/rule-demotion.js` applies the active list at the one place severities are finalised (`TestResult.addCheck`, `src/core/runner.js`) — demoted findings still appear, as warnings, with the reason attached, never hidden; the CLI prints "N rule(s) demoted by field silence data (see /noise)" when the list is non-empty. The list itself, `data/rule-demotions.json`, is generated by `scripts/generate-rule-demotions.js` from a noise snapshot over the same aggregator `/api/noise` uses (`website/app/lib/rule-noise.js`: `MIN_FINDINGS_FLOOR` = 50, `DEFAULT_MIN_SILENCED_RATE` = 0.2 — one definition, imported by both). No production ledger exists yet, so the checked-in list is empty and both the endpoint and the generator report `not-enough-data` rather than inventing a table — this move activates the moment 07's flywheel data exists. |
