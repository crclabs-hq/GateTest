...
## 2026-09-14 — Launch-readiness merge (PR #499, main `e101d517`) and the docs truth pass

**What merged (one integration merge, six verified fix branches plus two follow-ups, all landed 2026-09-15 UTC):** #492 WordPress plugin launch blockers (findings render, no key needed, honest $19 offer, zip + CI); #494 the Docker image builds again, stale self-scan badge, dead `.ai` address, guessed-URL 404s, www, module-count labels; #495 the seven Marketplace publish-surface blockers (tarball, a `cli` bin so bare `npx @gatetest/cli` resolves, the `v1` tag, action counts, the `permissions:` block, stale strings, version 1.61.1); #496 bad input stops passing green and the secrets rule stops blocking clean repos (django, got, self); #497 seven false positives closed against django, got and gin, each with the third-party line as its fixture; #498 `npm test` green and deterministic on main (Windows globbing, Node 24 leak reporting, three flaky files, one dead fixture — KI #109 closed); #503 vendor-neutral public copy with `tests/public-copy-vendor-neutral.test.js` as the guard; #504 Vapron → Tallrig (platform-config defaults, `MAIL_PROVIDER=tallrig`, the callback reads the dispatch secret through platform-config, `docs/ops/tallrig-cutover.md`).

**Measured after the merge (2026-09-15):** `node bin/gatetest.js --version` → 1.61.1; `npm view @gatetest/cli version` → 1.61.0 (1.61.1 unreleased, so the bare-npx form is not real on npm yet); `--list` → 121 modules (quick 42 / standard 46 / full 88 / nuclear 96 per `src/core/config.js`); `git rev-parse v1` → `e101d517`; `gatetest.io/api/platform-status` → 1.61.1 at `e101d517`, healthy; `/api/status` → `platform.name` Tallrig, `pointed_at` mixed (the box still dispatches through the `VAPRON_*` aliases); `github.com/marketplace/actions/gatetest-quality-gate` → 404 (not listed).

**Docs truth pass (the PR after #499):** README re-verified command by command — the Marketplace badge pointed at that 404 and now names the `v1` Action; `cd gatetest` after cloning `GateTest` fails on a case-sensitive filesystem → `cd GateTest`; the hosted-scan cap read "50 source files" while `route.ts` reads 60 for Quick and 4,000 for the engine tiers; the Vercel-era "serverless" wording is gone. The Bible's VERSION line says what npm serves; the "v1.61.2" heading no package ever carried is the dated 2026-09-05 entry; two `hello@` lines on the dead domain → `support@gatetest.io`; the MCP-tier paragraph agrees with the pricing table (local server free, `GATED_TOOLS` empty). `editors/vscode/` deleted with its only test (KI #68, extension-tree half — `vscode-extension/` is the one tree; shipping it is still Craig's call, the Fifty #49). Not closed by any of this: KI #105 (Code Scanning categories need an API delete), #107 (admin softening), #110 (cve-feed).

## 2026-09-10 — Legal & security pages rebuilt; production GitHub App wired; Marketplace identity corrected

**Production unblocked (Craig, on the box):** the real private key for GitHub App **3766251** (`gatetest-hq`) replaced the pasted documentation example that had killed App auth since 2026-08-06 (`invalid_placeholders` cleared; `scripts/ops/fire-test-webhook.js` completed a job end to end). `RESEND_API_KEY` set (MCP-tier key delivery). `GLUECRON_EMITTER_SECRET` set on the box and `GATETEST_URL` / `GATETEST_API_KEY` set in Gluecron's Vapron platform secrets — `/api/events/push` went 503 → 401 (authenticating). Lesson recorded in `src/core/github-app-permissions.js`: the website's `getPrivateKey()` needs the raw PEM (single line, `\n`-escaped), never a file path — only the engine's `src/app-server.js` accepts a path.

**Legal set (commit "feat(legal)"):** Terms (30 sections), Privacy (17), Data Processing Addendum (15, SCC Module 2 elections, generated Annex II), Sub-processors, Cookie Policy, rebuilt `/trust`, and RFC 9116 `public/.well-known/security.txt`. Every document is data (`*-content.ts`, all < 300 lines) rendered by `website/app/components/legal/LegalDocument.tsx` (anchor ids, contents list, cross-document footer). Every fact comes from `website/app/legal/_facts.js`; vendor and cookie tables are mapped from `F.SUB_PROCESSORS` / `F.COOKIES`; the GitHub App permission table is generated from `github-app-permissions.js` via `website/app/lib/legal-access-table.ts`. `tests/legal-facts.test.js` fails the suite if a stated retention window, the hosted-unsafe module list, the telemetry opt-out, or the sub-processor list drifts from the code that enforces it, and greps every legal source for DRAFT / TBD / "to be confirmed". The amber "Draft notice" banners and 22 attorney-review JSX comments are gone. Honesty lines held: standard 30-day AI-provider retention (not ZDR); AI-fix paths send complete files; findings retained until deletion is requested; no self-serve delete/export; no sandbox ("we do not execute your code"); no SOC 2 / ISO / pentest claims; Sentry session replay disclosed. Still owed by Craig: registered legal entity + postal address (`LEGAL_ENTITY` / `POSTAL_ADDRESS`), attorney review, and a look at the public commitments introduced (30-day deletion, 5-business-day disclosure acknowledgement, 72-hour GDPR breach notice). Footer and checkout now link the documents; checkout previously linked no legal page.

**Marketplace identity (commit "fix(marketplace)"):** every surface said 3322634 was live and told the operator to delete 3766251. Install button, preflight, docs and the sync test repointed; identity is one import (`APP_SLUG` / `APP_ID` / `appInstallUrl()`) guarded by `tests/github-app-identity.test.js`. Post-install and admin OAuth callbacks redirected to the box's private IP (`10.0.1.1:3000`) — fixed via `siteUrl()`. Preflight gained event / description / over-grant / external_url checks and now reports the live App's real gaps.

**Site fixes (commit "fix(website)"):** `/scan/status` with no id sat at 5% forever (Forbidden #4) — now a failed state; `/docs` 404 → redirects to `/developers`; global `:focus-visible` ring; `X-Powered-By` removed.

**Site shell (commit "feat(website): one site shell"):** Craig, later the same day — *"the design is completely different on every page… the navigation is really bad… this site should be a marketing beast… leading them to our git host Gluecron and the platform of the next decade Vapron."* Measured before the fix (Playwright over all 227 public routes × 3 widths): 208 of 227 pages rendered a dark page surface while home and pricing were light; three different navigation bars; the root layout rendered no header or footer at all. Now: `app/layout.tsx` renders `<SiteHeader/>`/`<SiteFooter/>` once (`components/SiteChrome.tsx`), navigation is one definition (`components/site-nav.ts`: Product · Solutions · Ecosystem(Gluecron, Vapron, /stack) · Docs · Pricing), pages use `PageHero` / `Section` / `StatTiles` and tokens only, dark accent panels use `bg-panel` tokens, the dark theme is opt-in (`data-theme="dark"`) until every component is token-based. Home: Repository / Website / WordPress hero tabs, the Gluecron + Vapron section moved up, a WordPress door added. `tests/site-shell.test.js` guards all of it. Measured after: every route has exactly one header and one footer, no horizontal scroll at 390px, nine click-through journeys green (menus, phone drawer, hero → running playground scan, tier CTA expand, checkout prefill, legal shell, scan-status no-id, install link). `/fixes` no longer 500s when its own API is unreachable.

**Copy fact-check (commit "fix(copy)"):** Craig, after the shell shipped — *"a lot of information on the site is incorrect which you didn't pick up in the render or journeys sweep."* Render and journey passes check layout and links, not truth. Three page-by-page audits checked every factual claim against the engine (registry, module code, `config.js` suites, `checkout-tiers.ts`, `engine-models.js`, the MCP tool list, `site-stats.json`, `legal/_facts.js`). The site had been describing a product we do not ship: a "CVE-feed" module that is not registered, an `authFlaws` module that does not exist, an `install-hook` command, a "blocking" pre-push hook that is advisory, Action inputs and a repo name that are wrong, `npx @gatetest/cli` (fails — three binaries; now `npx -p @gatetest/cli gatetest`), "no subscription" on a site selling two, every module "included on Full" when 34 are not, stale counts (90, 110, 67x), "never stored / read-only access / SOC 2–aligned" data claims the code does not back, and GitHub App pages underselling the free install (full deterministic engine on every push since 2026-08-18). Module pages now derive tier availability from `config.js` (`modules/[slug]/availability.ts`, `lib/module-suites.js`). Also: the "fad" band no longer names a model generation; selection colour inside dark panels fixed; Hall of Scans retired (`/scans` → `/precision`). Engine follow-up: `src/modules/cve-feed.js` is dead code and `lib/cve-to-fix.js` can never fire because no module emits CVE/GHSA ids.

**Known, not fixed tonight:** 27 engine-module test files fail on Windows only (backslash paths in expected finding ids, e.g. `a11y:html-lang:src\index.html`) — reproduced on a clean checkout of `c51f923b`; CI (Linux) is green. Live-site audit (`docs/` not updated; see session) also lists: home hero "68/85 modules" copy, 13-item navbar, per-page canonicals/titles on billing/checkout/status, CSP `unsafe-inline`/`unsafe-eval`, no dark theme.
...
|---|-------|----------|--------|
| 68 | **Two competing unshipped VS Code extension trees** (`editors/vscode/` JS + `vscode-extension/` TS, publisher GateTestHQ) with no canonical choice; site honestly says no extension ships yet (`how-it-works/page.tsx`). **ENGINE HALF DONE 2026-09-15:** `vscode-extension` 1.1.0 runs the engine in-process — `@gatetest/cli` is a bundled dependency loaded in a `worker_threads` worker (`vscode-extension/engine/engine-bridge.js` + `engine-worker.js`); the CLI spawn with the non-existent `--format json` / `--file` flags, the PATH lookup (Windows `spawn EINVAL`) and the global-settings write on activation are gone. Guarded by `tests/vscode-extension-engine-bridge.test.js` (fast) and `tests/heavy/vscode-extension-engine-worker.test.js` (real engine, fixture repo). `vsce package` builds a 6.3 MB vsix. Still unpublished — publishing to the VS Code Marketplace is Craig's (publisher `GateTestHQ` PAT). **MERGE REPAIR 2026-09-16:** #516 (CLI `--format json` / `--file`, extension spawning the shipped CLI) and #517 (in-process) landed the same night; the GitHub "Update branch" merge into #517 produced an invalid `vscode-extension/package.json` (duplicate commandPalette block, missing comma) and an `extension.ts` with both designs and duplicate functions, so every `npm ci` on main failed. Repaired on `hotfix/vscode-extension-merge-repair`: the extension is in-process (Craig's ask), #516's `addWorkspaceMcp` (.vscode/mcp.json) and its suite copy kept, the CLI's `--format json` / `--file` stay for other editors, and `tests/cli-format-json.test.js` now asserts the extension does NOT spawn the CLI. Also: `--ci-init gitlab\|circleci` generators exist (`src/core/ci-generator.js`) but are documented nowhere — README/site only show GitHub Actions. **CI-GENERATOR HALF DONE 2026-07-30.** `--ci-init github\|gitlab\|circleci` was verified working before being written up (all three exit 0 and emit conventional paths: `.github/workflows/gatetest.yml`, `.gitlab-ci.yml`, `.circleci/config.yml`; the generated configs are substantial, not stubs — quick diff-scoped scan on merge requests, full scan on main, separate security stage, JUnit+SARIF wired to native test reporting). Documented in README (new "Wire it into CI" section) and on `/quickstart`, whose GitLab/CircleCI answer had been telling people to hand-write a pipeline that a generator already writes for them. **Extension-tree half left for Craig:** choosing between `editors/vscode/` (JS, 6 files) and `vscode-extension/` (TS, 5 files) and deleting one is a product-direction decision about an unshipped surface, not cleanup — I am not deleting someone else's unshipped work on a LOW-severity row. | MEDIUM | **DONE (2026-09-16)** — one tree (`vscode-extension/`, editors/vscode deleted #505), engine in-process (#517 → #521 repair), Marketplace-safe metadata (#523), published as **GateTestHQ.gatetest 1.1.0** — https://marketplace.visualstudio.com/items?itemName=GateTestHQ.gatetest (validated, public 2026-09-16 04:36Z). CI generators for GitLab/CircleCI were documented 2026-07-30 (README + /quickstart). Left open on purpose: a new icon (1.1.1) and Cursor/Windsurf install verification. |
...
| 109 | **114 tests in 27 engine-module files fail on Windows only** (found 2026-09-10): expected finding ids are built with forward slashes (`a11y:html-lang:src/index.html`) while the module emits `path.relative()`'s raw answer, which carries the OS separator (`src\index.html` on Windows). Not a test problem — the same finding had two identities, so a `.gatetestignore` line or a `.gatetest/baseline.json` fingerprint written on Windows would not match on Linux CI and vice versa. | MEDIUM | DONE (2026-09-13) — one definition: `src/core/repo-path.js` (`repoRelative`, `toPosix`); 150 `path.relative` call sites in 86 files under `src/modules` + `src/core` routed through it by script (private `.replace(/\\/g, '/')` / `.split(path.sep).join('/')` suffixes dropped, 17 now-unused `path` requires removed); `finding-registry.normPath`, `ignore-file._normPath` and `baseline.fingerprint` delegate to `toPosix`; `tests/visual.test.js` was the one test that built its expectation with `path.sep` and now asserts the canonical id. 101 of the 114 went green on Windows; `SUITE: PASSED`, determinism check and quick self-scan green. |
...
| 112 | **`gatetest fix --apply --dry-run` wrote every fix to disk BEFORE printing "Would apply".** Found 2026-09-16 by reading the path end to end: `bin/gatetest.js` consulted `--dry-run` only after `runFixBatch` had returned, and `runFixOrchestration` writes the winning hypothesis (and swaps every candidate into the source path to run its tests, and a playback hit) on the way — the "plan" described changes already on disk, and `runFixBatch` then read the "would-be" content back FROM disk. Fix: `dryRun` is an orchestrator option — no winner write, no test swap-in (tests are reported **not checked**, Doctrine #1, rather than a pass never measured), no scratch dir, no flywheel event; the result carries `code` + `original` and `formatDryRunPlan` renders a per-file `+`/`-` diff. Control pair in `tests/cli-fix-orchestrator.test.js`: same fixture + same fake model — with `dryRun` every project file is byte-identical with its mtime pinned to 2001 unmoved and the plan names the change; without it the file is modified and the mtime moves. | HIGH | RESOLVED 2026-09-16 — three new tests green; the mtime assertion is proven non-vacuous by the apply-mode control. |
| 113 | **`enqueueScan` dropped `host` / `triggeredBy` and `/api/v1/scans` never wrote a row.** Found 2026-09-16 tracing the public API: `POST /api/v1/scans` called `enqueueScan` with `owner`/`repo`/`triggeredBy`/`metadata` and no `sql` — the store requires `repository`, `sha`, `sql` and had no `triggeredBy` at all — so it threw on every request, the catch logged it and the route still answered **201 "queued"**; `GET /api/v1/scans/:id` imported a `getScanByEventId` the store did not export, so every poll was a 404. Independently, the store coerced every host that was not `'github'` to `'gluecron'`, so an `'api'` row would have been labelled a Gluecron push and the worker would have fetched `https://gluecron.com/<hostname>/<path>` for five ticks then dead-lettered to Gluecron's callback. Fix: `KNOWN_HOSTS` (`gluecron`, `github`, `api`) is the one definition; `normalizeHost` keeps the documented default (`gluecron` when omitted) and **throws** on an unknown value instead of relabelling it as a trusted producer; `triggered_by TEXT` + `metadata JSONB` columns (idempotent `ADD COLUMN IF NOT EXISTS`), stored verbatim and returned by `claimNextJob` and the new `getScanByEventId(eventId, sql)`; the POST passes `sql`/`repository`/`sha`(digest of the URL)/`host:'api'`/`triggeredBy:'api_key:<id>'` and returns **503 QUEUE_UNAVAILABLE** when the row cannot be written; the GET reads the row with `getDb()`, maps queue `done`/`dead` to `completed`/`failed`, and returns `host` + `triggeredBy`; the worker marks non-`REPO_HOSTS` rows terminal with no scan and no callback. Control pairs: `tests/scan-queue-store.test.js` (host+triggeredBy+metadata persist as supplied; omitted → `gluecron`/null/null; `bitbucket` rejected with nothing INSERTed) and `tests/scan-worker-tick.test.js` (`api` row → terminal, 0 scans, 0 callbacks; identical `github` row → scanned). | HIGH | RESOLVED 2026-09-16 — 33 store tests, 42 worker/schema tests green. Not done: executing URL scans from the queue (Phase 2) — an `api` row is recorded and attributed, and the GET reports its terminal state honestly. **Phase 2 landed 2026-09-17** — the worker now tells the two `api`-row shapes apart by `metadata.url`: a git URL (github.com/gitlab.com) runs the SAME `runScan` path a repo-host job uses (same budget/time guards), `markDone`s the row, and records usage on the ledger under `apikey:<id>` via the existing `resolveAccountKey`/`recordUsage` (surface `'api'`) — no new identity scheme; a bare website URL is checked against `web-runtime-gate.js`'s own prerequisite list and, since the browser-runtime pass is not configured (and this synchronous one-job-per-tick worker cannot drive that gate's queued+polled dispatch to completion inline either way), the row is marked with the new terminal `not_checked` status (`markNotChecked`) and reason `web-runtime:not-configured` — never `done` with zero findings. No callback is ever sent for `api` rows (no Gluecron/GitHub consumer). `GET /api/v1/scans/:id` now imports one shared `publicStatusFor` (Doctrine #4, replacing a second hand-typed status table) that adds the `not_checked` public state and surfaces `reason` when present. Control pairs added: `tests/scan-queue-store.test.js` (`markNotChecked` stores the reason + empty findings, never `done`; `publicStatusFor` covers every internal status incl. `not_checked`), `tests/scan-worker-tick.test.js` (`api`+git URL → executed, `markDone`, usage recorded once, no callback; `api`+bare URL without the runtime configured → `markNotChecked`, no scan, no usage, no callback; `github` row unchanged), `tests/scan-worker-api-host.test.js` (`isGitRepoUrl` classifier: github.com/gitlab.com positive, lookalike domains and gluecron.com negative). **Phase 3 landed 2026-09-18** — gitlab.com is now actually fetched, not just recognised: `gluecron-client.ts`'s new `loadGitlabRepoFiles` reads a PUBLIC gitlab.com project (incl. subgroup paths, `group/subgroup/project`) via GitLab REST API v4's unauthenticated archive endpoint (`resolveGitlabDefaultBranch` first, since the archive endpoint has no "HEAD" alias like GitHub's codeload does), reusing `repo-snapshot.js`'s existing tar/gzip parser (GitLab's archive wraps entries in the same shape GitHub's does) via the new `fetchPublicGitlabSnapshot`/`gitlabTarballUrl` — no new dependency, no credential (self-hosted GitLab and private projects are out of scope by construction: `scan-executor.ts`'s `GITLAB_URL_RE` matches the `gitlab.com` hostname SEGMENT only). `scan-executor.ts`'s `runScan` now branches to a `runGitlabScan` BEFORE the two-segment owner/repo regexes (which would truncate a subgroup path); a 401/403/404 from gitlab.com — private, missing, or otherwise inaccessible — comes back `notChecked`/`gitlab:not-accessible` (`ScanResult`'s new fields) rather than a plain failure, and `runApiHostJob` (`scan-worker.js`) now marks that `not_checked` via the existing `markNotChecked` instead of retrying/dead-lettering a target that will answer identically forever; any other failure (network, timeout) keeps the normal retry/terminal classification every other host gets. Control pairs: `tests/repo-snapshot.test.js` (`fetchPublicGitlabSnapshot` — positive: subgroup path fetched and parsed; negative: 401/403/404 throw with `.httpStatus` set, a 500/network failure does not, so it is never mistaken for "not accessible"; project-path validation rejects `..` traversal segments) and its new `gitlab.com fetch-layer wiring` describe block (source-text contract on the TS files, same approach `tests/github-hardening.test.js` uses), plus `tests/scan-worker-tick.test.js` (a `notChecked` git-URL scan → `markNotChecked`, never `markFailed`/retried, no callback) and `tests/scan-worker-api-host.test.js` (`gitlab.com.evil.example`, `notgitlab.com`, and a self-hosted `gitlab.mycompany.com` all still miss the classifier). github.com path unchanged. |
| 107 | **Admin softening of the CI/CLI gate — investigated, and turned out to be a promise with no code behind it.** KI #107 had flagged the risk that this repo's `.gatetest.json` (`owner: "crclabs-hq"`, `admin: true`) could make our own CI gate advisory, in direct tension with Doctrine #9 (our own scanner must be able to block our own PR) and with Forbidden #25's own wording in CLAUDE.md ("admin paths ... auto-fix and pass"). **Reproduction 2026-09-16 found the risk had never materialized in code:** no file in the plain scan/gate path (`bin/gatetest.js`, `src/core/runner.js`) ever read `.gatetest.json`'s `admin`/`owner` keys or `process.env.GATETEST_ADMIN` — `config.js`'s own `KNOWN_ROOT_KEYS` comment says those keys are consumed *outside this process*. A synthetic blocking `secrets` finding on a fixture carrying this repo's exact admin config returned `GATE: BLOCKED` / exit 1 identically with and without `GATETEST_ADMIN=1` set — the flag was completely inert for gate enforcement; the only place admin softening existed in code was the customer-facing local pre-push template (`integrations/husky/pre-push`), never this repo's own CI. **Fixed per Forbidden #25's own recommendation on file:** `src/core/admin-override.js` is the one definition (Doctrine #4) of the softening decision, imported by `runner.js` and `json-output.js`. `.gatetest.json`'s `admin`/`owner` keys NEVER soften the gate — `admin: true` was removed from this repo's `.gatetest.json` (`owner` stays; it is a legitimate no-op for this decision, read by other consumers per `config.js`). ONLY `GATETEST_ADMIN=1` in the environment of the current run can soften a BLOCKED result, no workflow in `.github/workflows/` sets it, and it is never silent (Forbidden #16): `runner.js` prints a loud `GATE SOFTENED` notice and every summary carries `adminOverride` + `rawGateStatus`, which `json-output.js` exposes so an editor/CI consumer sees the override too, not just the terminal. Documented for legitimate local use: one line in `.env.example`. Control pair in `tests/admin-override.test.js` (unit-level runner + spawned real CLI against a fixture with this repo's exact `.gatetest.json` shape): (a) blocking finding, no env var → `GATE: BLOCKED`, exit 1; (b) same finding, `GATETEST_ADMIN=1` → exit 0, stderr AND the JSON contract both name the env var and say `SOFTENED`, and the finding is still reported, never hidden. | MEDIUM | RESOLVED 2026-09-16 — softening is now real, opt-in only via the env var, and never silent; `.gatetest.json` no longer carries a repo-wide `admin: true`. The `prSize` dogfood-depth half of this KI was already DONE 2026-09-05 (`fetch-depth: 0` on the Self-Scan job). |
| 112 | **Issue #630 follow-ups G3/G4/G5, surfaced by the same reporter, not fixed by the #630 PR (a second, unrelated KI already used #112 above — see `docs/ROADMAP.md`'s Known Issues numbering; this row is the G3/G4/G5 item, not the dry-run one).** **G3** — `--json` had whole-run `duration` but no per-module timing. **G4** — `.gatetest.json`'s `ignore` key was accepted and read by nobody; a customer whose build pipeline templates the JSON file but doesn't also hand-maintain `.gatetestignore` got silent no-op suppression. **G5** — `envVars` checked "does an `.env.example` exist" once at the repo ROOT and applied that answer to every workspace; on Tallrig's 76-workspace platform monorepo this turned 291 of 1,081 findings in unrelated packages into blocking errors. Plus one item added to the same PR: **`.gatetest.json`'s 13-unread-keys warning was stderr-only**, invisible to `--format json` and the PR comment. All four RESOLVED 2026-09-22: G5 — `.env.example` is resolved per the OWNING workspace (`src/core/workspaces.js`, the one definition); a miss only blocks when the reading package itself ships its own `.env.example` and fell through it, proven on a synthetic 3-workspace fixture (2 blocking → 0 blocking, 1 → 3 warnings). G4 — `ignore-file.js`'s `load()` takes the config array as extra lines into the SAME parser, no second matcher; `runner.js` reads `config.get('ignore')`. G3 — `doc.modules.list` carries `{module, status, durationMs}` per module, sourced from the same `TestResult.duration` the console reporter already prints. Unknown-keys — `GateTestConfig#getUnknownKeysCheck()` is `null` when clean (nothing emitted) or a `config:unknown-keys` info-severity, never-blocking finding naming every key with a "did you mean X?" hint or an honest "no equivalent in this version"; threaded onto `summary.configCheck`, read by both `console-reporter.js` and `json-output.js`. Control pairs in `tests/env-vars.test.js`, `tests/ignore-file.test.js`, `tests/runner-suppression.test.js`, `tests/cli-format-json.test.js`, `tests/config-unknown-keys.test.js`. | MEDIUM | RESOLVED 2026-09-22 — all three original defects plus the config-check addition; corpus (`scripts/real-world-precision.js`) and determinism (`scripts/determinism-check.js`) re-run clean. |
