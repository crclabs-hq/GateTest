...
### Addendum 2026-09-26 12:15Z — main green again; outside crawl of gatetest.io (AlecRae session) sets the next fix order
- **Main is green at 0ee079b6:** the owner squash-merged #764 (listing copy + handoff + board rows), which carried the changelog TypeScript fix; CI, Docker publish and the box deploy all passed and `/api/platform-status` reports commit 0ee079b, `lastPullDeploy: up-to-date` at 11:45Z. #761 closed as superseded. Open with auto-merge, all merged up to main: #760, #763, #773, #774; plus #775/#776 from the AlecRae session (issues #769, #767).
- **Outside first-hour crawl, 11:33–11:50Z, unauthenticated** (raw evidence: the AlecRae session's scratchpad file `crawl-gatetest/FINDINGS-gatetest-2026-09-26.md`; 200 page renders, 214 internal links all 200, six security headers everywhere, zero Vapron). Verdict as a buyer: install the free tools, do not pay for Scan+Fix yet — the site's own proof says 0/40. Findings and owners:
  - **GT-01 P0** `/testing` renders AUTO-FIXED 0 % (0/40) in success-green; arena has 359 bot bug PRs since June and zero fixer PRs; root cause arena issue #374: `401 API key is invalid`. **Owner rotates `ANTHROPIC_API_KEY` on gatetest-arena.** Code half in flight: builder on `fix/public-proof-honesty` (warning tone + honest sentence); #760 puts the error text on the console so the workflow's grep fires; arena PR 379 (merged 12:04Z) fixes README `gatetest.ai` → `.io` (GT-05) and `CLAUDE_MODEL` → `claude-sonnet-5`.
  - **GT-02 P1** `/api/status` unauthenticated lists missing secret names, Stripe mode, queue, pointing. Builder on `fix/status-endpoint-disclosure`: minimal public body, full body behind admin session or the CRON_SECRET bearer; same PR adds a JSON catch-all for `/api/*` and `/health`/`/healthz` rewrites (GT-09) and fixes the pricing link to code scanning (GT-06, 404 because code scanning is not enabled on this repo).
  - **GT-03 P1** public `/status`: hosted scans Degraded, worker "no job activity, last 16 days ago", App webhooks last accepted 16 days ago. Box 161 tick timers; SSH closed to sessions — **owner** (`systemctl list-timers gatetest-tick.timer gatetest-watches.timer`).
  - **GT-04 P1** `/changelog` publishes handoff commit subjects ("fixer does not fix"). Builder on `fix/public-proof-honesty`: generator excludes docs(handoff)/docs(board)/chore(stats)/chore(trainer) and docs-only merges; JSON regenerated.
  - **GT-07/08/10 P2** logged for the next pass: 19 px tap targets on nav/footer, `/noise` and `/precision` "no data yet" (nightly corpus run populates `rules[]`), CSP `unsafe-inline unsafe-eval`.
- **12:40Z:** #774 (move 12) merged. **#778 open** (GT-01 + GT-04): `website/app/lib/testing-proof-tone.ts` decides success vs warning (0/N, or last fix older than 7 days → warning with an honest sentence; "—" always labelled); `scripts/generate-changelog.js` gained `isChangelogWorthy()` — handoff/board/stats/trainer titles and docs-only merges are excluded (150 → 132 entries). Judgment call for the owner: plain doc-only commits (a README fix) are now excluded from the public changelog too. #773 re-merged with #774: the PR comment and JSON summary carry `deferred`/`budgetLimited` and `rootCause` together (85/85).
- **13:00Z:** **#779 open** (GT-02 + GT-09 + GT-06): `/api/status` unauthenticated → `{ok, healthy, version, commit, checked_at}` only; full body behind `isAdminRequest` or the `CRON_SECRET` bearer (`isAuthorisedTick`), and `public-status-collect.ts` passes the bearer server-side so `/status` keeps rendering; `website/app/api/[...notFound]/route.ts` returns JSON 404s and `next.config.ts` rewrites `/health` + `/healthz` to `/api/health`; `SAMPLE_REPORT_URL` (`website/app/lib/sample-report.ts`, used by /pricing and the free-scan result) moved from the code-scanning view (404 anonymous even with scanning enabled) to the CI run list for main (200 anonymous). 134/134 + 3/3, tsc and eslint clean. Every crawl finding that is code (GT-01/02/04/05/06/09) now has a PR or is merged; GT-03 and the arena key remain the owner's; GT-07/08/10 next pass.
- **Classifier note for every session:** in auto mode the classifier denies `gh pr merge --admin` and the `update-branch` API on this repo. A green PR that is only BEHIND is merged by the owner; sessions bring their own branches up to date with a local `git merge origin/main` and push.
