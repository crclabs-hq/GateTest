...
|---|---|---|---|
| **VS Code Marketplace** | `GateTestHQ.gatetest` 1.1.3 | item page 200 (public); 1.1.3 uploaded 2026-09-16, gallery re-verification of that version pending | Publisher `GateTestHQ` under ccantynz@gmail.com. Update = manage page → row ⋮ → Update, or `publish-vscode.yml` once `VSCE_PAT` exists. First upload (1.1.0) was refused as "suspicious content" until the metadata lost threat language + competitor keywords (#523). |
| **Open VSX** — the registry **Cursor, Windsurf, VSCodium, Gitpod, Eclipse Theia** read | `GateTestHQ/gatetest` 1.1.3 — https://open-vsx.org/extension/GateTestHQ/gatetest | **LIVE 2026-09-16**: `https://open-vsx.org/api/GateTestHQ/gatetest` returns 200 with version 1.1.3; extension page 200 | Namespace `GateTestHQ` exists. Publishing is automated by `.github/workflows/publish-vscode.yml` with the `OVSX_PAT` repo secret (the workflow creates the namespace if missing, then `ovsx verify-pat` + `ovsx publish` of the same `.vsix` the Marketplace gets, on the same run). Install: search **GateTest** in the editor's Extensions view, or open the listing. |
...
| 4 | **Docker — ghcr.io/crclabs-hq/gatetest** | `docker run` users; the Dockerfile builds green in CI | `docker-publish.yml` pushes only on a release tag | Falls out of #1. |
| 5 | **WordPress.org plugin directory** | The `/wp` audience searches there first | Zip not on a release; directory review required | After #1 attaches the zip: wordpress.org/plugins/developers/add → upload `gatetest-health-check.zip` (readme.txt `Stable tag` must match the plugin version) → review takes days–weeks. |
| 6 | **PulseMCP / Smithery / mcp.so** | Discovery for MCP servers | PulseMCP lists us at the dead `.ai` domain | PulseMCP: submit a correction (form). Smithery: `smithery.ai/new` with the repo URL. mcp.so: submit form. All need a human account. |
| 7 | **JetBrains Marketplace** | IntelliJ / WebStorm users | No plugin exists | Build item, not a listing step. |

## What is scripted already

- `.github/workflows/publish-vscode.yml` — package + publish to the VS Code Marketplace (`VSCE_PAT`) and Open VSX (`OVSX_PAT`); dry-run by default. The Open VSX leg is live as of 2026-09-16: `OVSX_PAT` is set, the `GateTestHQ` namespace exists (the workflow's `ovsx create-namespace` step treats "already exists" as success), and 1.1.3 is on open-vsx.org.
