...
| 4 | Time-to-verdict contract: ETA first, per-module timing, `--budget` degrades and reports | DONE | PR #773 |
...
| 18 | Weekly public head-to-head vs Semgrep, CodeQL, Sonar on the same corpus | PARTIAL | Verified 2026-09-26: cron is weekly (Mon 05:00 UTC) and firing — runs 2026-09-19 (dispatch) and 2026-09-21 (schedule); rolling PR `flywheel/head-to-head` opened and auto-merged both times (#614, #622); site data is dated 2026-09-21 (5 days old). Both runs' own GH conclusion was FAILURE from one stale assertion in `tests/head-to-head.test.js` (expected CodeQL always "not measured") — already fixed on `main` in `5179e0` before this session; unconfirmed by an actual green run until the next Monday (2026-09-28). This session closed the page gap: `stalenessSentence()` added to `website/app/lib/head-to-head.js`, rendered on `/precision` once the table passes 14 days old; date-of-run, per-repo pinned SHA, and SonarQube/CodeQL "not measured" + reason were already present. No compare/pricing/blog page claims "weekly" for head-to-head, so nothing there was false. Owner item: confirm the 2026-09-28 scheduled run is green (this session could not wait on it). |
