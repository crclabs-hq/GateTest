...
| 4 | Time-to-verdict contract: ETA first, per-module timing, `--budget` degrades and reports | DONE | #773 merged 2026-09-26 — ETA line, per-module timing store, `--budget` deferral disclosed on console/JSON/PR comment |
...
| 12 | Root cause on every red run: classifier verdict, blame commit, replay command | DONE | #774 merged 2026-09-26 — `src/core/root-cause.js`: why / since / replay on console, JSON, SARIF and the PR comment |
...
| 16 | Convergence guard on fix and crawl loops; never re-flag own fix | DONE | #750 merged 2026-09-26 — `src/core/convergence-guard.js` |
