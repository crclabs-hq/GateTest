 ...
 *
 * Covers:
 *   - BaseModule._collectFiles honours the per-instance incremental context
...
describe('BaseModule._collectFiles — incremental filter', () => {
  it('returns ALL files when no incremental context is set (default behaviour)', () => {
  ...
  it('filters to ONLY incremental files when context is set', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gatetest-collect-'));
    try {
      ...
      const m = new BaseModule('test', 'test');
      m._incrementalContext = {
        changedFilesAbs: new Set([path.resolve(a), path.resolve(c)]),
      };
      const files = m._collectFiles(dir, ['.js']);
  ...
  it('preserves the extension filter alongside the incremental filter', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gatetest-collect-'));
    try {
      const a = write(dir, 'a.js', '');
      ...
      const m = new BaseModule('test', 'test');
      m._incrementalContext = {
        changedFilesAbs: new Set([path.resolve(a), path.resolve(b)]),
      };
      // Even though b.txt is in the set, it must be dropped by extension filter
      const files = m._collectFiles(dir, ['.js']);
      assert.deepStrictEqual(
...
describe('universal-checker.runLanguageChecks — incremental filter', () => {
  ...
  it('empty Set is treated as "no filter" (full scan)', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gatetest-uc-'));
    try {
      write(dir, 'a.py', 'eval("1")\n');
