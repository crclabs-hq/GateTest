...
describe('MCP server — check_health', skipMcpOnWindows, () => {
  it('returns operational status with 95 modules', async () => {
    ...
    assert.ok(text.includes('95'), `expected 95 modules in health output: ${text}`);
  });
...
describe('MCP server — list_modules', skipMcpOnWindows, () => {
  it('returns a list containing 95 modules', async () => {
    const res = await callMcp('tools/call', { name: 'list_modules', arguments: {} });
    const text = res.result.content[0].text;
    assert.ok(text.includes('95'), `expected 95 modules count: ${text.slice(0, 200)}`);
  });
