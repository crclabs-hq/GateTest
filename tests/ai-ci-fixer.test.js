...
test('buildPrBody and buildIssueBody render the expected content', () => {
  ...
  assert.match(pr, /_Generated by GateTest_/);
  assert.doesNotMatch(pr, /Claude|Anthropic/, "the CI-fixer PR body must not name the AI vendor");
...
test('buildPrBody: renders Files changed section with source tags', () => {
  ...
  assert.match(pr, /`AI` `c\.js`/);
  assert.doesNotMatch(pr, /`Claude`/, 'the file tag names the layer, not the AI vendor');
});
...
test('flywheel: distillClaudeWins calls auto-distill for every patched file', () => {
  ...
  assert.equal(distilled[0].provenance.originalModel, 'claude-sonnet-5');
});
