...
const { controlPairCounts } = require(path.join(ROOT, 'src', 'core', 'control-pairs'));
...
/**
 * Per-rule aggregate across the corpus (the Fifty, move 02): a per-repo
 * precision table proves little to a senior developer skimming for false
 * positives — a per-rule table with the corpus sha and control-pair evidence
 * is what gets checked. Built from the same `allFindings` this run already
 * collects for calibration (repo -> error-severity findings with rule,
 * confidence and module), so nothing new is measured, only aggregated.
 *
 * `onZeroCeilingRepos` counts findings (blocking OR softened) that landed on
 * a repo whose ceiling is 0 — our cleanest control repos. A rule that fires
 * only there, even softened, is the shape of a false positive; a rule that
 * never touches a zero-ceiling repo has never been wrong on code we call
 * clean. `controlPairs` comes from `src/core/control-pairs.js` — the one
 * definition of the `// control-pair: <ruleId>` marker.
 * @param {Array<{name: string, findings: Array<{rule:string, confidence:number, module:string|null}>|null}>} allFindings
 * @param {{repos: Array<{name:string, sha:string, maxBlocking?:number}>}} manifest
 * @param {Map<string, number>} controlPairsByRule
 */
function aggregateRulesByCorpus(allFindings, manifest, controlPairsByRule) {
  const repoByName = new Map(manifest.repos.map((r) => [r.name, r]));
  const zeroCeilingRepos = new Set(manifest.repos.filter((r) => r.maxBlocking === 0).map((r) => r.name));
  const byRule = new Map();

  for (const repo of allFindings) {
    if (!Array.isArray(repo.findings)) continue; // not measured on this run
    const meta = repoByName.get(repo.name);
    for (const f of repo.findings) {
      if (!byRule.has(f.rule)) {
        byRule.set(f.rule, { rule: f.rule, module: f.module || null, findings: 0, onZeroCeilingRepos: 0, repos: [] });
      }
      const entry = byRule.get(f.rule);
      entry.findings += 1;
      if (zeroCeilingRepos.has(repo.name)) entry.onZeroCeilingRepos += 1;
      if (meta && !entry.repos.some((r) => r.name === meta.name)) entry.repos.push({ name: meta.name, sha: meta.sha });
    }
  }

  return [...byRule.values()]
    .map((r) => ({ ...r, controlPairs: controlPairsByRule.get(r.rule) || 0 }))
  ...
  if (opts.writeJson) {
    if (measured.length !== repos.length) {
      ...
      const out = {
        ...
        engineCommit,
        repos: measured,
        // Per-rule aggregate (the Fifty, move 02) — same run, same findings,
        // grouped by rule instead of by repo. Empty (not omitted) when no
        // findings were retained, so the page can say "awaiting the nightly"
        // instead of silently rendering nothing.
        rules: aggregateRulesByCorpus(allFindings, manifest, controlPairCounts(path.join(ROOT, 'tests'))),
        rulesNote: 'Per-rule findings across the corpus, with the count on repositories held to a zero ceiling '
          + '(our cleanest controls) and how many tests under tests/ are tagged as that rule\'s control pair. '
          + 'Same run as the table above; regenerated by the nightly dogfood scan, not on every merge.',
