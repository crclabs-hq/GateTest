...
export const ALL_TOOLS = [
  ...
  { name: "capture_screenshot", paid: true, desc: "👁 Eyes — screenshot any live URL or localhost so the AI sees the rendered page" },
  { name: "get_visual_diff", paid: true, desc: "👁 Eyes — baseline vs current visual diff to catch UI regressions" },
  { name: "run_live_checks", paid: true, desc: "👂 Ears — runtime errors, console warnings, API health on any URL" },
  { name: "get_production_errors", paid: true, desc: "👂 Ears — top Sentry / Datadog / Rollbar errors with file:line attribution" },
  { name: "verify_fix", paid: true, desc: "🤝 Hands — re-scan changed files for hard pass/fail proof the fix worked" },
