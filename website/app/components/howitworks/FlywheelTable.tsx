...
const LAYERS: Layer[] = [
  {
    ...
    name: "Recipe",
    cost: "$0.00",
    description: "Deterministic replay of fixes the AI layer has already proven. A recipe is distilled from a small, templatey AI diff and replays only after it has been confirmed enough times to be promoted to stable — an unproven patch never auto-applies.",
    wins: "Repeat shapes the AI layer has solved before, once their recipe has earned promotion. Zero cost, zero model call.",
    example: {
  ...
  },
  {
    ...
    name: "AI fix",
    cost: "paid",
    description: "The AI fix engine proposes a minimal, surgical diff for the finding — deeper analysis on the paid fix tiers. On the CI path it generates three competing hypotheses in a single call and the best-ranked candidate wins. Capped per tier so spend never exceeds margin.",
