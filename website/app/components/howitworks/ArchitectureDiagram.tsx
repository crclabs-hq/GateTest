...
const NODES: Node[] = [
  ...
  { id: "engine", label: `Worker fetches job  →  Gate runs ${TOTAL_MODULES} modules`, detail: "deterministic, no AI by default", y: 310, variant: "engine" },
  ...
  { id: "fix", label: "Fix engine  ·  Recipe → AI → Gates", detail: "patch must clear syntax + re-scan gates", y: 510, variant: "fix" },
  { id: "gate", label: "Test gen  +  syntax gate  +  scanner re-validation", detail: "broken fixes never reach the PR", y: 610, variant: "fix" },
  { id: "review", label: "Pair review  +  architecture annotation  ·  Tier 2+", detail: "a second AI reviewer critiques every fix", y: 700, variant: "review" },
  { id: "nuclear", label: "Correlation  +  AI diagnosis  +  executive summary  ·  Forensic", detail: "attack chains across findings, per-finding diagnosis, CISO report (mutation + chaos run via GitHub Action)", y: 790, variant: "nuclear" },
  ...
  return (
    <div className="w-full overflow-x-auto -mx-2 px-2">
      <svg
        ...
        role="img"
        aria-label={`GateTest scan pipeline architecture: customer push enters via GitHub App or Gluecron Signal Bus, lands in a Postgres scan queue, runs through ${TOTAL_MODULES} deterministic modules, clusters and ranks findings, replays a promoted fix recipe or asks the AI fix engine for a fix, validates each fix through a syntax gate and scanner re-validation, adds pair review and architecture annotation for Tier 2 and up, adds cross-finding correlation, per-finding AI diagnosis, and executive summary for Tier 3 (mutation testing and chaos / fuzz pass are available via the GitHub Action where a CI runner is present), and finally opens a pull request.`}
