        ...
        <div key={category.id} id={`modules-${category.id}`} className="scroll-mt-24">
          <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-2 mb-4">
            <h3 className="text-xl font-semibold text-foreground">
              {category.title}
              <span className="ml-2 text-sm text-muted font-normal font-mono">
            ...
            <p className="text-sm text-muted max-w-md">{category.blurb}</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {category.modules.map((mod) => (
              <details
                key={mod.name}
                className="group rounded-xl border border-border bg-surface-light p-4 transition-colors hover:border-border-strong hover:bg-accent/5 open:border-accent/20 open:bg-accent/10"
              >
                <summary className="cursor-pointer list-none flex items-start justify-between gap-3">
                  <div>
                    <div className="font-mono text-sm font-semibold text-foreground">{mod.name}</div>
                    <p className="text-xs text-muted mt-1 leading-relaxed">{mod.description}</p>
                  </div>
                  <span
                    aria-hidden="true"
                    className="text-muted text-xs font-mono shrink-0 mt-0.5 group-open:rotate-45 transition-transform"
                  >
                    +
                  </span>
                </summary>
                <div className="mt-3 pt-3 border-t border-border">
                  ...
                  <code className="block text-xs text-foreground-secondary font-mono leading-relaxed break-words">
                    {mod.example}
                  </code>
                </div>
              </details>
