...
export const CWE_TOP_25: CweEntry[] = [
  ...
  },
  {
    ...
    remediation: "Use SameSite=Lax or SameSite=Strict on session cookies. Verify CSRF tokens on state-changing requests. cookieSecurity flags sameSite: 'none' cookies that are not also marked secure, plus httpOnly: false and secure: false. webHeaders flags a missing X-Frame-Options / frame-ancestors policy.",
  },
  {
    ...
    remediation: "Validate MIME type AND file magic-bytes (don't trust the extension). Store outside the web root. Rename to a hash on save. Set a strict size limit. Never serve uploads from a directory where they can be executed. GateTest has no upload-specific rule today; crossFileTaint catches the adjacent path-traversal shape when the uploaded filename reaches a filesystem sink.",
  },
  {
  ...
  },
  {
    ...
    modules: ["authBypass", "secrets", "cookieSecurity"],
    example: "if (req.body.password == storedPassword) { ... } — timing-attackable + no rate limit + no hashing.",
    remediation: "Use a vetted auth library (Lucia, NextAuth, Devise, Spring Security). Hash passwords with bcrypt/argon2. Rate-limit login attempts. Verify JWT signatures and audience. authBypass flags route handlers with no authentication signal, secrets flags hardcoded credentials and JWTs, and cookieSecurity flags placeholder session secrets and insecure session-cookie flags.",
  },
  {
  ...
  },
  {
    ...
    remediation: "Don't accept serialised native objects from the network. Use JSON for data interchange. If you must deserialise, use schemas (Avro, Protobuf with explicit types) and validate. GateTest's python module flags pickle loads; Java/PHP native deserialisation is not covered by a dedicated rule today.",
  },
  {
  ...
  },
  {
    ...
    remediation: "Require authentication on every state-changing endpoint by default; make public endpoints opt-in. Audit your routing table for missing middleware. authBypass scans Express, Next.js App Router and Fastify handlers and flags any with no auth signal — middleware, session read, token check — unless the route is explicitly marked public.",
  },
  {
  ...
  },
  {
    ...
    remediation: "Never use eval / new Function with user input. For dynamic logic, use a sandboxed expression evaluator (mathjs, jsep) with an explicit allowed-function list. security flags eval() in production code and crossFileTaint traces user input across files into eval / exec sinks.",
  },
  {
    ...
    remediation: "Authorisation belongs server-side, against persisted state. Never accept role / permission claims from the client. Use a vetted policy library (Casbin, OPA). GateTest's authBypass module covers missing authentication (CWE-306), not wrong authorisation logic — this class needs a policy review and a two-account test today.",
  },
  {
