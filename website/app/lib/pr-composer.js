...
function renderAttemptHistoryTable(attemptHistoryByFile) {
  ...
  return [
    ...
    '',
    '| File | Attempts | Outcomes | AI time | Final |',
    '| --- | --- | --- | --- | --- |',
    ...rows,
  ].join('\n');
}
...
function renderRegressionTests(fixes) {
  ...
  return [
    ...
    '',
    `The fix engine wrote ${tests.length} new regression test${tests.length !== 1 ? 's' : ''}. Each one demonstrates the original bug would have failed and the fix passes — so reverting any of these fixes will trip the new test.`,
    '',
    ...lines,
  ].join('\n');
}
...
function composePrBody(opts) {
  ...
  sections.push(`Every fix in this PR was generated by the AI fix engine and validated through three gates before commit: per-file iterative re-attempt loop, cross-fix syntax check, and cross-file scanner re-validation. Fixes that failed any gate were rolled back automatically — they don't ship to your branch.`);
  ...
  sections.push('2. **Iterative fix loop** — for every finding, the fix engine attempts a fix, GateTest re-checks the file, and if quality drops, the AI sees the failure and tries again. Up to N attempts (default 3).');
  ...
  sections.push('5. **Regression tests** — for every fix, the fix engine writes a test that would have failed against the buggy code. Tests ship in the same PR.');
